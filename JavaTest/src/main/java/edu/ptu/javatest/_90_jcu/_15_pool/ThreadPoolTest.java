...
public class ThreadPoolTest {
    ...
    @Test
    public void testCacheThreadPool() {//new ThreadPoolExecutor(0, Integer.MAX_VALUE, 60L, TimeUnit.SECONDS, new SynchronousQueue<Runnable>());
        ExecutorService executorService = Executors.newCachedThreadPool();//60秒回收线程
        for (int i = 0; i < 100; i++) {
            new Thread(() -> executorService.execute(() -> {

            })).start();
        }
        int createThreadNum = getThreadPoolWorkers(executorService).size();
        try {
            Thread.sleep(TimeUnit.SECONDS.toMillis(30));
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
        Assert.assertEquals(createThreadNum, getThreadPoolWorkers(executorService).size());//alive
        try {
            Thread.sleep(TimeUnit.SECONDS.toMillis(33));
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
        Assert.assertEquals(0, getThreadPoolWorkers(executorService).size());//destroy
    }
...
//            System.out.print("  "+poolExecutor.toString().substring(poolExecutor.getClass().getName().length())+" ");
            ...
            System.out.println("  ; thread work " + getThreadPoolWorkers(poolExecutor).size());//需要看队列是否允许添加task
        }
        try {
            Thread.sleep(5000);
        } catch (InterruptedException e) {

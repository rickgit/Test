...
//        UserModel.User.parseFrom();
        ...
        builder.setAddr(ProtoModel.Address.newBuilder().setName("shenzhen").build());
        ...
        Assert.assertEquals(new String(Arrays.copyOfRange(bytes,4,4+"rick".length())), "rick" );
        //address:tlv
        Assert.assertEquals(bytes[4+"rick".length()], toProtoByteTag(3,2));//embedded messages
        Assert.assertEquals(bytes[9], 10);//embedded messages 长度
        ///addr name :tlg
        Assert.assertEquals(bytes[10], toProtoByteTag(1,2));//addr tag
        Assert.assertEquals(bytes[11], "shenzhen".length());//addr tag
        Assert.assertEquals(new String(Arrays.copyOfRange(bytes,12,12+"shenzhen".length())), "shenzhen" );

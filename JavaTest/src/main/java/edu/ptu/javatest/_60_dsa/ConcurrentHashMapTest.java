...
public class ConcurrentHashMapTest {
    ...
    public void testGrow() {
        ConcurrentHashMap<Object, Object> chm = new ConcurrentHashMap<>();
        for (int i = 0; i < 9; i++) {
            chm.put(new HashMapTest.HashObj(i), "");
        }
        printTree(chm);
    }
    ...
    public void testThread() {
        ...
        for (int i = 0; i < 129; i++) {
            ...
            executorService.execute(() -> {
                Thread.yield();
                chashmap.put(new HashMapTest.HashObj(Integer.parseInt(stringBuffer.substring(0,1).toLowerCase())), new Random(23).nextInt(23));
//                printTree(chashmap);
            });
        }
        try {

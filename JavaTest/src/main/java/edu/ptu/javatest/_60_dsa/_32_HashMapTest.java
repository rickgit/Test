...
//        printArrayNode(map, 1);
    ...
    @Test
    public void testcomparableClassFor() {//如果不可以比较，直接用一致性id比较
        Object stringKey = _00_ReflectionTest.invokeMethod(HashMap.class, HashMap.class, "comparableClassFor", new Class[]{Object.class}, new Object[]{new String()});
        Assert.assertNotNull(stringKey);

        Object keyComparaOtherType = _00_ReflectionTest.invokeMethod(HashMap.class, HashMap.class, "comparableClassFor", new Class[]{Object.class}, new Object[]{new HashObjComparable2()});
        Assert.assertNull(keyComparaOtherType);

        Object keyComparaSelfType = _00_ReflectionTest.invokeMethod(HashMap.class, HashMap.class, "comparableClassFor", new Class[]{Object.class}, new Object[]{new HashObjComparable(1, 2)});
        Assert.assertNotNull(keyComparaSelfType);
    }

    @Test
    public void testcompareComparables() {//如果不可以比较，直接用一致性id比较
        HashObjComparable hashObjComparable = new HashObjComparable(1, 2);
        Object keyComparaSelfType = _00_ReflectionTest.invokeMethod(HashMap.class, HashMap.class, "comparableClassFor", new Class[]{Object.class}, new Object[]{hashObjComparable});
        Assert.assertNotNull(keyComparaSelfType);


        HashObjComparable2 hashObjComparable2 = new HashObjComparable2();
        Assert.assertNotEquals(hashObjComparable2.getClass(), hashObjComparable.getClass());
        int value = (int) _00_ReflectionTest.invokeMethod(HashMap.class, HashMap.class, "compareComparables"
                , new Class[]{Class.class, Object.class, Object.class}, new Object[]{keyComparaSelfType, hashObjComparable, hashObjComparable2});
        Assert.assertEquals(0, value);
    }

    public static class HashObjComparable2 implements Comparable<Integer> {
        @Override
    ...
    public static class HashObjComparable implements Comparable<HashObjComparable> {//必须用泛型接口，才会走compareTo。HashMap#comparableClassFor()
        int hash;
        int value;

        HashObjComparable(int hash, int value) {
            this.hash = hash;
            this.value = value;
        }
        ...
        @Override
        public int compareTo(HashObjComparable hashObjComparable) {
            return hashObjComparable == null ? value : (value - hashObjComparable.value);
        }
    }
    ...
    public void testPrintRBTree() {
        HashMap hashMapTest = new HashMap(64);
        for (int i = 0; i < 9; i++) {
            hashMapTest.put(new HashObjComparable(1, i*2), i*2);
        }
        Object[] table = (Object[]) getRefFieldObj(hashMapTest, hashMapTest.getClass(), "table");
        printTreeNode(table[1]);
        hashMapTest.put(new HashObjComparable(1,7), 7);
        printTreeNode(table[1]);
        System.out.println();
    }

    public static void main(String[] args) {
        new _32_HashMapTest().testPrintRBTree();
    }
